...
export function CocktailsSection() {
  ...
  return (
    <section ref={ref} id="coqueteis" className="relative overflow-hidden bg-white px-5 py-20 md:px-8 lg:py-24 xl:py-28">
      <motion.div className="relative z-[2] mx-auto max-w-[1500px]" initial="hidden" animate={controls} variants={fadeInUp} transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.8, ease: easeOut }}>
        <div className="grid gap-10 border-b border-[#c8b98e]/45 pb-12 lg:grid-cols-[0.68fr_0.32fr] lg:items-end">
          ...
          <div className="flex lg:justify-end max-lg:justify-center">
            <MagneticLink href={cocktailsContent.href} className="group inline-flex items-center gap-3 bg-beggin-red px-8 py-4 font-serifDisplay text-[12px] font-bold uppercase tracking-[0.18em] text-white transition hover:bg-beggin-ink xl:text-[13px]">
              <span>Descubra mais!</span><ArrowRight size={16} />
            </MagneticLink>
          </div>
        </div>
        ...
        <div className="mt-12 space-y-8 lg:space-y-10">
          {cocktails.map((cocktail, index) => {
            ...
            return (
              <Link key={cocktail.id} href={cocktail.href} className="group grid overflow-hidden border border-[#c8b98e]/60 bg-white shadow-[0_20px_54px_rgba(54,43,25,0.055)] transition duration-300 hover:border-beggin-red/60 hover:shadow-[0_28px_70px_rgba(54,43,25,0.09)] lg:grid-cols-2">
