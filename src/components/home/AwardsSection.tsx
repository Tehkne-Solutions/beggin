...
function AwardsTitle({ title }: { title: string }) {
  if (title.includes('Medalha de Ouro')) return <>Medalha de Ouro<br />no Brasil<br />e no Mundo.</>;
  return <>{title}</>;
}
...
export function AwardsSection() {
  ...
  const stripVariants = { hidden: { opacity: 0, y: 24, scale: 0.98 }, visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.75, ease: 'easeOut' } } };

  return (
    <section ref={sectionRef} id="premios" className="relative overflow-hidden bg-white px-0 py-16 lg:py-18">
      <div className="relative mx-auto grid w-[calc(100%-44px)] max-w-[1540px] items-center gap-10 border-y border-[#c8b98e]/30 py-12 lg:w-[calc(100%-64px)] lg:grid-cols-[0.28fr_0.72fr] lg:py-14 xl:gap-12">
        ...
        <motion.div className="relative z-[3] w-full pb-2" animate={controls} initial="hidden" variants={stripVariants}>
          <div className="mx-auto grid max-w-[720px] grid-cols-2 items-center justify-items-center gap-5 sm:grid-cols-3 lg:max-w-[760px] lg:grid-cols-3 xl:max-w-[820px] xl:gap-6 2xl:flex 2xl:max-w-none 2xl:items-center 2xl:justify-center 2xl:gap-6">
            {featuredMedals.map((medal) => <div key={medal.src} className="relative h-[128px] w-[128px] sm:h-[148px] sm:w-[148px] lg:h-[154px] lg:w-[154px] xl:h-[168px] xl:w-[168px] 2xl:h-[188px] 2xl:w-[188px]"><Image src={medal.src} alt={medal.alt} fill sizes="(max-width: 640px) 128px, (max-width: 1024px) 148px, (max-width: 1536px) 168px, 188px" className="object-contain drop-shadow-[0_14px_22px_rgba(54,43,25,0.13)]" /></div>)}
          </div>
        </motion.div>
      </div>
    </section>
